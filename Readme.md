...
```
(mkdir -p /tmp/dots && cd /tmp/dots && curl -L https://github.com/jeffcressman/dots/archive/master.tar.gz | tar zx --strip 1 && sh ./install.sh)
```
...
- install homebrew
- installs binaries (python, sshfs, git, etc.)
- sets OSX defaults
- installs applications via `homebrew-cask` (lastpass, atom, virtualbox, vagrant, iterm2, etc.)
